...
contains
  ...
  subroutine read_grid_data()
    ...
    ! 读取网格维度
    open(101, file='data_mat/B_grid_info.dat', status='old')
    read(101, *) nx, ny, nphi
    read(101, *) R0_axis
    close(101)
    ...
    ! 读取 R 网格
    open(102, file='data_mat/R_grid.dat', status='old')
    do ix = 0, nx
      do iy = 0, ny
    ...
    ! 读取 Z 网格
    open(103, file='data_mat/Z_grid.dat', status='old')
    do ix = 0, nx
      do iy = 0, ny
    ...
    ! 读取 phi 网格
    open(104, file='data_mat/phi_grid.dat', status='old')
    do iphi = 1, nphi
    ...
    ! 读取 psi 和 rho 网格
    open(105, file='data_mat/psi_grid.dat', status='old')
    do ix = 0, nx
  ...
  subroutine read_field_data()
    ...
    ! 读取 B_R
    open(106, file='data_mat/B_R.dat', status='old')
    do ix = 0, nx
      do iy = 0, ny
        do iphi = 1, nphi
    ...
    ! 读取 B_Z
    open(107, file='data_mat/B_Z.dat', status='old')
    do ix = 0, nx
      do iy = 0, ny
        do iphi = 1, nphi
    ...
    ! 读取 B_phi
    open(108, file='data_mat/B_phi.dat', status='old')
    do ix = 0, nx
      do iy = 0, ny
        do iphi = 1, nphi
  ...
  subroutine output_results()
    ...
    open(201, file='data_mat/lyapunov_results.dat')
